...
try{
	...
	if(sub.equals("Ingresar Entrada")){
		String id_inv="", det_pro="";
		int tipo_en=0, c1=0, c2=0, c3=0;
		c2=Integer.parseInt(request.getParameter("txtf_cant"));
		...
		if (tipo_en==1){//Insumo existente
			...
			stmt.execute("insert into kardex values ('0', '0', '"+det_pro+"', '"+c3+"', 'ENTRADA POR AJUSTE', '-', NOW(), '"+request.getParameter("txta_razon")+"', '"+request.getParameter("id_usu")+"', '0')");
		} else {//Insumo inexistente
			String cla_uni="", detalle = "";
			stmt.execute("insert into detalle_productos values ('0', '"+request.getParameter("txtf_clave2")+"', '"+lot+"', '"+df.format(df2.parse(cad))+"', '"+request.getParameter("cla_fin")+"', '"+request.getParameter("ori")+"', '0')");
			...
			rset=stmt.executeQuery("select det_pro from detalle_productos where cla_pro = '"+request.getParameter("txtf_clave2")+"' and lot_pro = '"+lot+"' and cad_pro = '"+df.format(df2.parse(cad))+"' and cla_fin = '"+request.getParameter("cla_fin")+"' and id_ori = '"+request.getParameter("ori")+"' ");
			while(rset.next()){
				detalle=rset.getString("det_pro");
			}
			stmt.execute("insert into inventario values (NOW(), '"+cla_uni+"', '"+detalle+"', '"+request.getParameter("txtf_cant")+"', '0', '0')");
			rset=stmt.executeQuery("select p.cla_pro, p.des_pro, dp.lot_pro, dp.cad_pro, i.cant, i.id_inv, o.des_ori, dp.det_pro FROM usuarios us, unidades u, inventario i, detalle_productos dp, productos p, origen o where  us.cla_uni = u.cla_uni and u.cla_uni = i.cla_uni AND i.det_pro = dp.det_pro and dp.cla_pro = p.cla_pro AND dp.id_ori = o.id_ori and us.id_usu = '"+request.getParameter("id_usu")+"' and p.cla_pro='"+request.getParameter("txtf_clave2")+"' and dp.lot_pro='"+lot+"' and dp.cad_pro='"+df.format(df2.parse(cad))+"' ");
			while(rset.next()){
				tipo_en=1;
				id_inv=rset.getString("id_inv");
				det_pro=rset.getString("det_pro");
...
<table width="666" height="346" border="3" align="center" cellpadding="2" bgcolor="#FFFFFF">
  <tr>
    ...
    <td width="650"><form id="form" name="form" method="post" action="carga_clave.jsp?id_usu=<%=request.getParameter("id_usu")%>">
      <table width="650" height="377" border="0" align="center" cellpadding="2">
        ...
        <tr>
          ...
          <td colspan="2" class="style5"><input name="txtf_lote" type="text" id="txtf_lote" onChange="mayApemm(this.form)" onKeyPress="return handleEnter(this, event)" value="<%=lot%>" size="10" />
        ...
        <tr>
          ...
          <td colspan="2" class="style5"><input name="txtf_cad" type="text" id="txtf_cad" onChange="mayApemm(this.form)" onKeyPress="return handleEnter(this, event)" value="<%=cad%>" size="10" />
  ...
  <tr>
        ...
        <tr>
          ...
          <td colspan="2"><div align="center">
            <input type="submit" name="submit" value="Ingresar Entrada" onClick="return checaRazon();" />&nbsp;&nbsp;<!--input type="submit" name="Submit" value="Limpiar Claves" /-->
